...
def get_user_queries(user_id):
    resp = supabase.table("queries").select("*").execute()
    print(resp.data)

    # Fetch user details from users table based on user_id
    user_resp = supabase.table("users").select("username, email").eq("user_id", user_id).execute()
    ...
    # Fetch queries for the matched user_id (from users table)
    resp = supabase.table("queries").select("query_text, timestamp").eq("user_id", user_id).execute()
    queries = resp.data or []
    
    if not queries:
        return f"<p>No queries found for {user_name} ({user_email}).</p>"

    html = f"<h3>Queries for {user_name} ({user_email})</h3><ul>"
    for q in queries:
        html += f"<li>{q['query_text']} (at {q.get('timestamp', 'N/A')})</li>"
    html += "</ul>"
...
def get_user_responses(user_id):
    ...
    # Fetch responses for the matched user_id (from users table)
    resp = supabase.table("responses").select("response_text, timestamp").eq("user_id", user_id).execute()
    responses = resp.data or []
    
    if not responses:
        return f"<p>No responses found for {user_name} ({user_email}).</p>"

    html = f"<h3>Responses for {user_name} ({user_email})</h3><ul>"
    for r in responses:
        html += f"<li>{r['response_text']} (at {r.get('timestamp', 'N/A')})</li>"
    html += "</ul>"
