...
def get_user_queries(user_id):
    try:
        ...
        app.logger.error(f"Error fetching queries: {e}")
        return jsonify({"error": "Internal Server Error"}), 500if 'admin_id' not in session:
        app.logger.error("Unauthorized access: No admin_id in session")
        return jsonify({'status': 'error', 'message': 'Please log in as admin'}), 401
    try:
        app.logger.debug(f"Session: {session}, User ID: {user_id}")
        query_resp = supabase.table("queries").select("*").eq("user_id", user_id).execute()
        app.logger.debug(f"Queries response for user_id {user_id}: {query_resp}")
        if query_resp.get('error'):
            error_msg = f"Supabase error: {query_resp['error'].get('message', 'Unknown error')}"
            app.logger.error(error_msg)
            return f"<p>{error_msg}</p>"
        queries = query_resp.get('data', [])
        
        if not queries:
            user_resp = supabase.table("users").select("*").eq("user_id", user_id).single().execute()
            if user_resp.get('error'):
                error_msg = f"Supabase error fetching user: {user_resp['error'].get('message', 'Unknown error')}"
                app.logger.error(error_msg)
                return f"<p>{error_msg}</p>"
            user = user_resp.data
        ...
        html = "<h3>Queries</h3><ul>"
        for q in queries:
            query_text = q.get('query_text', 'No text')
            timestamp = q.get('timestamp', 'No timestamp')
            if not isinstance(query_text, str) or not isinstance(timestamp, str):
                app.logger.error(f"Invalid data format for query: {q}")
                raise ValueError("Invalid data format in queries table")
            html += f"<li>{query_text} (at {timestamp})</li>"
        html += "</ul>"
        return html
    except Exception as e:
        error_msg = f"Exception in get_user_queries: {str(e)}\nStack Trace: {''.join(logging.traceback.format_exc())}"
        app.logger.error(error_msg)
        return f"<p>{error_msg}</p>"
...
def get_user_responses(user_id):
    ...
    try:
        ...
        app.logger.debug(f"User response for user_id {user_id}: {user_resp}")
        if user_resp.get('error'):
            error_msg = f"Supabase error fetching user: {user_resp['error'].get('message', 'Unknown error')}"
            app.logger.error(error_msg)
            return f"<p>{error_msg}</p>"
        ...
        app.logger.debug(f"Responses response for user_id {user_id}: {responses_resp}")
        if responses_resp.get('error'):
            error_msg = f"Supabase error: {responses_resp['error'].get('message', 'Unknown error')}"
            app.logger.error(error_msg)
            return f"<p>{error_msg}</p>"
        responses = responses_resp.get('data', [])
        ...
        for r in responses:
            response_text = r.get('response_text', 'No text')
            timestamp = r.get('timestamp', 'No timestamp')
            if not isinstance(response_text, str) or not isinstance(timestamp, str):
                app.logger.error(f"Invalid data format for response: {r}")
                raise ValueError("Invalid data format in responses table")
            html += f"<li>{response_text} (at {timestamp})</li>"
        html += "</ul>"
        return html
    except Exception as e:
        error_msg = f"Exception in get_user_responses: {str(e)}\nStack Trace: {''.join(logging.traceback.format_exc())}"
        app.logger.error(error_msg)
        return f"<p>{error_msg}</p>"
