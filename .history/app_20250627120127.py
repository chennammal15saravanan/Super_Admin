...
def (user_id):
    resp = supabase.table("queries").select("user_id","query_text").eq("user_id", user_id).execute()
    return render_template("user_datasets.html", queries=resp.data or [], user_id=user_id)

# User uploads view
@app.route('/users/<user_id>/responses')
def user_responses(user_id):
    resp = supabase.table("responses").select("user_id,response_text").eq("user_id", user_id).execute()
    return render_template("user_uploads.html", responses=resp.data or [], user_id=user_id)
