...
def get_user_queries(user_id):
    if 'admin_id' not in session:
        ...
        return jsonify({'status': 'error', 'message': 'Please log in as admin'}), 401
    try:
        app.logger.debug(f"Processing request for user_id: {user_id}, Session: {session}")
        # Verify user exists
        ...
        user = user_resp.data
        queries_resp = supabase.table("queries").select("*").eq("user_id", user_id).execute()
        if queries_resp.get('error'):
            error_msg = f"Supabase error fetching queries: {queries_resp['error'].get('message', 'Unknown error')}"
            app.logger.error(error_msg)
            return jsonify({'status': 'error', 'message': error_msg}), 500
        ...
        if not queries:
            ...
            return jsonify({'status': 'success', 'message': f'No queries found for {username} ({email})', 'data': []}), 200

        # Format queries for response
        queries_list = [
            {'query_text': q.get('query_text', 'No text'), 'timestamp': str(q.get('timestamp', 'No timestamp'))}
            for q in queries
        ]
        return jsonify({'status': 'success', 'data': queries_list}), 200
...
def get_user_responses(user_id):
    ...
    try:
        ...
        if responses_resp.get('error'):
            error_msg = f"Supabase error fetching responses: {responses_resp['error'].get('message', 'Unknown error')}"
            app.logger.error(error_msg)
            return jsonify({'status': 'error', 'message': error_msg}), 500
        responses = responses_resp.get('data', [])

        if not responses:
            return jsonify({'status': 'success', 'message': f'No responses found for {user["username"]} ({user["email"]})', 'data': []}), 200

        # Format responses for response
        responses_list = [
            {'response_text': r.get('response_text', 'No text'), 'timestamp': str(r.get('timestamp', 'No timestamp'))}
            for r in responses
        ]
        return jsonify({'status': 'success', 'data': responses_list}), 200
