...
def get_user_queries(user_id):
    try:
        ...
        if query_result.data and len(query_result.data) > 0:
            ...
            # Fetch user info for better message
            user_result = supabase.table("users").select("username, email").eq("user_id", user_id).single().execute()
...
def get_user_responses(user_id):
    try:
        ...
        response_result = supabase.table("responses").select("response_text").eq("user_id", user_id).execute()

        print(f"[DEBUG] Supabase result: {response_result}")
        print(f"[DEBUG] Response Data: {response_result.data}")

        if response_result.data and len(response_result.data) > 0:
            content = "<strong>Responses:</strong><ul>"
            for item in response_result.data:
                content += f"<li>{item.get('response_text', 'No text')}</li>"
            content += "</ul>"
            return content
        else:
            user_result = supabase.table("users").select("username, email").eq("user_id", user_id).single().execute()
            ...
            username = user_data.get("username", "Unknown User")
            email = user_data.get("email", "")
            return f"<strong>Responses</strong><br>No responses found for {username} ({email})."
...
def test_fetch_responses(user_id):
    try:
        print(f"[TEST] Checking responses for user_id: {user_id}")
        ...
        print(f"[ERROR] Test fetch failed: {str(e)}")
