...
def get_user_queries(user_id):
    print("✅ Fetching queries for user_id:", user_id)

    user_resp = supabase.table("users").select("*").eq("user_id", user_id).execute()
    print("🧾 User fetch result:", user_resp.data)
    ...
    print("📥 Queries fetch result:", queries_resp.data)

    queries = queries_resp.data or []

    if not queries:
        return f"<p>No queries found for {user['username']} ({user['email']}).</p>"

    html = f"<h3>Queries for {user['username']} ({user['email']})</h3><ul>"
    for q in queries:
        html += f"<li>{q['query_text']} (at {q['timestamp']})</li>"
    html += "</ul>"
    return html
