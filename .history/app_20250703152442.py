...
def get_user_responses(user_id):
    try:
        ...
        result = supabase.table("responses").select("response_text, timestamp").eq("user_id", user_id).execute()

        print("[DEBUG] Response Query Raw Result:", result.data)

        if result.data:
            content = "<strong>Responses:</strong><ul>"
            for r in result.data:
                response_text = r.get("response_text", "No response")
                timestamp = r.get("timestamp", "N/A")
                content += f"<li><strong>{timestamp}</strong><br>{response_text}</li><hr>"
            content += "</ul>"
            return content
        else:
