...
def get_user_queries(user_id):
    query_resp = supabase.table("queries").select("query_text").eq("user_id", user_id).execute()
    queries = query_resp.data

    if not queries:
        user_resp = supabase.table("users").select("username", "email").eq("user_id", user_id).single().execute()
        user = user_resp.data
...
def get_user_responses(user_id):
    response_data = supabase.table("responses").select("response_text").eq("user_id", user_id).execute()
    ...
    if not responses:
        ...
        username = user.get("username", "User")
        email = user.get("email", "")
        return f"<p>No responses found for {username} ({email}).</p>"

    html = "<ul>"
    for r in responses:
        html += f"<li>{r['response_text']}</li>"
    html += "</ul>"
    return html
