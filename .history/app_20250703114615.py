...
def get_user_queries(user_id):
    if 'admin_id' not in session:
        return jsonify({'status': 'error', 'message': 'Please log in as admin'}), 401
    try:
        logging.debug(f"Session: {session}, User ID: {user_id}")
        query_resp = supabase.table("queries").select("*").eq("user_id", user_id).execute()
        logging.debug(f"Queries response for user_id {user_id}: {query_resp}")
        ...
        for q in queries:
            html += f"<li>{q.get('query_text', 'No text')} (at {q.get('timestamp', 'No timestamp')})</li>"
        html += "</ul>"
        return html
...
def get_user_responses(user_id):
    ...
    try:
        ...
        logging.debug(f"User response for user_id {user_id}: {user_resp}")
        if user_resp.get('error'):
            error_msg = f"Supabase error fetching user: {user_resp['error'].get('message', 'Unknown error')}"
            logging.error(error_msg)
            return f"<p>{error_msg}</p>"
        ...
        logging.debug(f"Responses response for user_id {user_id}: {responses_resp}")
        ...
        for r in responses:
            html += f"<li>{r.get('response_text', 'No text')} (at {r.get('timestamp', 'No timestamp')})</li>"
        html += "</ul>"
        return html
    except Exception as e:
