...
def dashboard():
    ...
    resp = supabase.table("users").ilike("user_id, username, email").execute()
    users = resp.data or []
...
def get_user_queries(user_id):
    try:
        ...
        query_result = supabase.table("queries").select("query_text").ilike("user_id", user_id).execute()
        ...
        if query_result.data and len(query_result.data) > 0:
            ...
            user_info = supabase.table("users").select("username, email").ilike("user_id", user_id).single().execute().data
