...
def get_user_queries(username):
    ...
    user_resp = supabase.table("users").select("user_id, username, email").eq("username", username).execute()
    print(f"User response data: {user_resp.data}")
    ...
    query_resp = supabase.table("queries").select("query_text, timestamp").eq("user_id", user_id).execute()
    queries = query_resp.data or []
    if not queries:
