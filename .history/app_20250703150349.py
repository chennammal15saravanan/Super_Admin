...
def get_user_queries(user_id):
    try:
        ...
        result = supabase.table("queries").select("query_text, timestamp").eq("user_id", user_id).execute()
...
def get_user_responses(user_id):
    user_id = str(UUID(user_id.strip()))

    try:
        print(f"[INFO] Fetching responses for user_id: {user_id}")
        user_id = str(UUID(user_id.strip()))  # clean and validate UUID

        result = supabase.table("responses").select("*").eq("user_id", user_id).execute()

        print(f"[DEBUG] Supabase Raw Result: {result}")
        ...
        if result.data:
            ...
            return f"<strong>Responses</strong><br>No responses found for {user_data.get('username', 'Unknown')} ({user_data.get('email', '')})."

    except Exception as e:
        print("[ERROR] An exception occurred:")
        traceback.print_exc()  # ← This gives full traceback in terminal
