...
app.secret_key = 'your-secret-key'  # Required for session management, change this in production
CORS(app)
...
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InN3cmhjc2Z1b3JqcXN6cW9vaGZiIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTA5MTgwNzAsImV4cCI6MjA2NjY0OTQwNzd9.Th0bFIOF5Xrrk6oSQKeRGxDwYSJxkcJvksxQla4vIZM"
...
def users():
    ...
    try:
        ...
        if resp.get('error'):
            error_msg = f"Supabase error: {resp['error'].get('message', 'Unknown error')}"
        ...
        error_msg = f"Exception in /users route: {str(e)}"
        logging.error(error_msg)
        return jsonify({'status': 'error', 'message': error_msg}), 500
