...
@app.route("/users")
def users():
    resp = supabase.table("admins").select("id, username, email").execute()
    users = resp.data or []
    return render_template("users.html", users=users)

@app.route('/users/<user_id>/datasets')
def user_datasets(user_id):
    resp = supabase.table("datasets").select("dataset_id, dataset_name, created_at")\
        .eq("user_id", user_id).execute()
    return render_template("user_datasets.html", datasets=resp.data or [], user_id=user_id)

app.route('/users/<user_id>/uploads')
def user_uploads(u@ser_id):
    resp = supabase.table("file_uploads").select("file_id, file_name, upload_time")\
        .eq("user_id", user_id).execute()
